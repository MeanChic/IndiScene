...
<body>
		...
		<input type="button" value="글쓰기 " onclick="location.href='${root}/placeBoard/write.do'"/>
		...
		<c:set var="image" value="${requestScope.mainImageList[s.count - 1]}" /> 
			...
			<div class="form_style" style="height:130px;">
				<div class="disp" style="border-width:1px;">
					<a href="${root }/placeBoard/read.do?board_num=${market.board_num}&pageNumber=${pageNumber}"><img style="height:75px; width:100px" src="${image}" /><br>${market.subject }<br/></a>
		...
		<c:if test="${count>0 }">
			...
			<c:if test="${startPage>pageBlock }">
				<a href="${root }/placeBoard/enterBoard.do?pageNumber=${startPage-pageBlock }">[이전]</a>
			</c:if>
			
			<c:forEach var="i" begin="${startPage }" end="${endPage }">
				<a href="${root }/placeBoard/enterBoard.do?pageNumber=${i}">[${i }]</a>
			</c:forEach>
			
			<c:if test="${endPage<pageCount }">
				<a href="${root }/placeBoard/enterBoard.do?pageNumber=${startPage+pageBlock }">[다음]</a>
			</c:if>
		</c:if>
