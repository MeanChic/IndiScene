			...
			<div class="form-controlBoard">
				...
				<label class="boardlabelBlock marketBoardSize46 label-color marketBoardAttribute01 marketBoardY-location02">
					...
					<c:if test="${list.muse_name == null}">
						<a class="label-color" href="javascript:uploadRead('${root}','${list.board_num}','1')">${list.subject}</a>
					</c:if>
					<c:if test="${list.muse_name != null}">
						<a class="label-color" href="javascript:museRead('${root}','${list.board_num}','1','${muse_name}')">${list.subject}</a>
					</c:if>
				</label>
			...
			<c:if test="${count>0 }">
				<c:if test="${count>0 }">
					...
					<c:if test="${startPage>pageBlock }">
						<li><a aria-label="Previous" href="javascript:myCollaboMusic('${root }','${artist_id}','${startPage-pageBlock}')"><span aria-hidden="true">&laquo;</span></a></li>
					</c:if>
					
					<c:forEach var="i" begin="${startPage }" end="${endPage }">
						<li><a href="javascript:myCollaboMusic('${root }','${artist_id}','${i}')">${i}</a></li>
					</c:forEach>
					
					<c:if test="${endPage<pageCount }">
						<li><a aria-label="Next" href="javascript:myCollaboMusic('${root }','${artist_id}','${startPage+pageBlock}')"><span aria-hidden="true">&raquo;</span></a></li>
					</c:if>
				</c:if>
			</c:if>
		</ul>
