	...
	<center>
			<c:if test="${count>0 }">
				...
				<c:if test="${startPage>pageBlock }">
					...
					<a href="javascript:myUploadMusic('${root }','${artist_id}','${startPage-pageBlock}')">[이전]</a>	
				</c:if>
				
				<c:forEach var="i" begin="${startPage }" end="${endPage }">
					<a href="javascript:myUploadMusic('${root }','${artist_id}','${i}')">[${i}]</a>
				</c:forEach>
				
				<c:if test="${endPage<pageCount }">
					<%-- <a href="${root }/freeBoard/list.do?pageNumber=${startPage+pageBlock}">[다음]</a> --%>
					<a href="javascript:myUploadMusic('${root }','${artist_id}','${startPage+pageBlock}')">[다음]</a>				
				</c:if>
			</c:if>
		</center>
	</c:if>
