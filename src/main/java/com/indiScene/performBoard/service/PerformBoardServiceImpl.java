...
public class PerformBoardServiceImpl implements PerformBoardService {
	...
	@Autowired
	private PerformBoardDao performBoardDao;
	...
	public void findZipcode(ModelAndView mav) {
		...
		if(dong!=null){
			list=performBoardDao.ZipcodeList(dong);
	...
	@date  : 2015. 6. 25.
	@auther: 김정승
	@description :공연게시판 리스트를 호출하는 서비스 Dao
	 */
	@Override
	public void enterBoard(ModelAndView mav) {
		...
		int count=performBoardDao.getCount();
		...
		List<PerformBoardDto> performList=null;
		List<String> mainImageList=null;
//		HashMap<String,performBoardDto> hMap=null;
//		List<HashMap<String,performBoardDto>> mapList=null;
		
		if(count >0) {
			performList=performBoardDao.getMarketList(startRow,endRow);
			logger.info("performList" + performList.size());
			
			mainImageList=commonIOService.mainImageFind(performList); //첫번째 이미지를 뽑아오는 메소드
			//hMap=new HashMap<String,performBoardDto>();
			
		}
		
		mav.addObject("list", performList);
		mav.addObject("count", count);
		mav.addObject("boardSize", boardSize);
		mav.addObject("pageNumber",currentPage);
		mav.addObject("mainImageList",mainImageList);
		mav.setViewName("performBoard/enterBoard");
	}
	
	
//	@Override   //CommonIOService로 독립함
//	public List<String> mainImageFind(List<performBoardDto> performList) {
//		List<String> mainImageList=new ArrayList<String>();
//		String contentForImg=null;
//		for(int i=0;i<performList.size();i++){
//			contentForImg=performList.get(i).getContent();
...
//	}
	...
	public void write(ModelAndView mav) {
		// TODO Auto-generated method stub
		Map<String, Object> map=mav.getModelMap();
		PerformBoardDto performBoardDto=(PerformBoardDto)map.get("performBoardDto");
		MultipartHttpServletRequest request=(MultipartHttpServletRequest)map.get("request"); 
		Date date=new Date();
		
		logger.info("--" + request.getParameter("d_day1")+" "+request.getParameter("hour"));
		SimpleDateFormat sdf = new SimpleDateFormat("MM/dd/yyyy hh : mm");
		performBoardDto.setRegister_date(date);
		try {
			performBoardDto.setD_day((Date)sdf.parse(request.getParameter("d_day1")+" "+request.getParameter("hour")));
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		commonIOService.multiFileUpload(request,performBoardDto); //뒤에 Dto는 CommonDto로 받는다
...
//				}
//			}
//			performBoardDto.setFile_name(fileNameForDto);
//			performBoardDto.setFile_path(filePathForDto);
//			
...
//		//멀티파일 처리 끝
		...
		int check=performBoardDao.insert(performBoardDto);
	...
	public void read(ModelAndView mav) {
		...
		PerformBoardDto performBoardDto=performBoardDao.read(boardNumber);
		
		//한줄댓글을 불러온다
		List<ReplyDto> replyList=replyDao.replyList(boardNumber);
		
		int countCheck=0;
		if(performBoardDto !=null){
			countCheck=performBoardDao.count(boardNumber);
		}		
		
		logger.info("performBoardDto:" + performBoardDto);
		...
		mav.addObject("performBoard",performBoardDto);
		mav.addObject("pageNumber",currentPage);
...
//		
//		String attachDir="resources/performResources/img/"; //콜백으로 넘길용도+실제 파일 들어갈 용도
...
//	}
	...
	public void delete(ModelAndView mav) {
		...
		mav.setViewName("performBoard/delete");
	...
	@date  : 2015. 6. 25.
	@auther: 김정승
	@description :공연게시판 글을 삭제하기 위한 서비스 메소드
	 */
	@Override
	public void deleteOk(ModelAndView mav) {
		...
		String realPass=performBoardDao.passCheck(artist_id);
		...
		if(password.equals(realPass)){
			...
			PerformBoardDto performBoardDto=performBoardDao.read(board_num);
			
			commonIOService.multiFileDelete(performBoardDto); //독립된 파일처리 과정을 호출
			//다중파일 삭제 처리 시작
//			if(performBoardDto.getFile_name()!=null){    //CommonService로 독립
//				File file=null;
//				String fileNameForDto=performBoardDto.getFile_name();
//				String filePathForDto=performBoardDto.getFile_path();
...
//			}
			...
			delCheck=performBoardDao.delete(board_num);
		...
		mav.setViewName("performBoard/deleteOk");
	...
	public void update(ModelAndView mav) {
		...
		PerformBoardDto performBoardDto=performBoardDao.update(board_num);
//		logger.info(performBoardDto.toString());
//		logger.info(performBoardDto.getContent());
		mav.addObject("board",performBoardDto);
		mav.addObject("pageNumber",pageNumber);
	...
	@date  : 2015. 6. 25.
	@auther: 김정승
	@description :공연게시판 수정을위한 서비스 메소드
	 */
	public void updateOk(ModelAndView mav) {
		...
		if(performBoardDto.getFile_name()!=null){
			
		}
		SimpleDateFormat sdf = new SimpleDateFormat("MM/dd/yyyy");
		
		try {
			performBoardDto.setD_day((Date)sdf.parse(request.getParameter("d_day1")));
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		int check=performBoardDao.updateOk(performBoardDto);
//		System.out.println(updateCheck);
		mav.addObject("check",check);
		mav.addObject("board_num",board_num);
		mav.addObject("pageNumber",pageNumber);
		mav.setViewName("performBoard/updateOk");
